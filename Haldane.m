...
knum=101;
x_frac=linspace(0,1,knum+1);
y_frac=linspace(0,1,knum+1);
x_frac=x_frac(1:knum);
y_frac=y_frac(1:knum);
[X_frac,Y_frac]=meshgrid(x_frac,y_frac);
Kx=X_frac.*g.b(1,1)+Y_frac.*g.b(2,1);
Ky=X_frac.*g.b(1,2)+Y_frac.*g.b(2,2);
Kz=zeros(knum,knum);
...
Ky=X_frac.*g.b(1,2)+Y_frac.*g.b(2,2);
Kz=zeros(knum,knum);
[Omega_k,KX,KY,KZ]=MTB.ham.get_Berrycurvature_cop(g,Kx,Ky,Kz,Enk,Unk,1:2,0.03,1);

%% Calculate slab bands
[nbands,~,nrpts]=size(g.ham); %nbands-number of bands; nrpts-number of r points
MillerIndices=[1,0,0];
g_slab=g;
Umatrix=g_slab.MillerIndicestoumatrix(MillerIndices);
Urot=g_slab.surfab;
...
efermi=0;
nslab=100
[Energy,kpath,kindex]=MTB.ham.get_slab_bands(g_slab.ham,g_slab.hopr2,nslab,nbands,nrpts,hkpoints,nk,g_slab.a2,g_slab.b2);
MTB.plot.plot_bands(Energy,nbands,efermi,kpath,labels,kindex,"Haldane",0)
...
y_frac=y_frac(1:knum);
[X_frac,Y_frac]=meshgrid(x_frac,y_frac);
Kx=X_frac.*g_slab.b2(1,1)+Y_frac.*g_slab.b2(2,1);
Ky=X_frac.*g_slab.b2(1,2)+Y_frac.*g_slab.b2(2,2);
