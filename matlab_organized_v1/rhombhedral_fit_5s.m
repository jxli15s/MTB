...
%%
filename='data/Graphene/5s/wb_tb.dat';
...
ham_s=zeros(nbands*2,nbands*2,nrpts);
for idx=1:nrpts
    % ham_s(:,:,idx)=kron(g.ham(:,:,idx),s0);
    ham_s(:,:,idx)=kron(s0,g.ham(:,:,idx));
end

g.ham=ham_s;
g.atoms=kron([1,1]',g.atoms);
g.wpos=kron([1,1]',g.wpos);
