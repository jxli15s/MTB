...
% efermi=-0.117;
%%
Electric_field_in_evpA=0.00/d0; %0.08-0.12 V/A
...
efermi=Energy(15,251)/2+Energy(16,251)/2;
% efermi=0
MTB.plot.plot_bands(Energy,nbands,efermi,kpath,labels,kindex,"bulk",0)
% plot(kpath(251),Energy(15,251),'r*')
g.iniham=g.ham+0;
ylim([-0.1,0.1])
...
bands_data(:,2)=bands_data(:,2)-3.0349;
...
lb = [V_pi-1 , V_sigam-1, t2-1, -1,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 下界
ub = [V_pi+1,  V_sigam+1, t2+1, 1,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 上界
...
win1 = 0.018;   % 上界
win2 = 0.0;   % 下界
mask_E = (E_DFT > win2) & (E_DFT < win1);   % 开区间
weights(mask_E) = 500;
...
for i=1:size(bands_vasp,2)
    plot(kpath_vasp(:,i),bands_vasp(:,i),'r-',LineWidth=2)
    plot(kpath_vasp(:,i),bands_sk(:,i)-0.0022,'b--',LineWidth=2)
end
ylim([-1,1])
