...
clear;
% ==== 1. 读取 linewidth 文件 ====
% linewidth_file = 'data/BaCoAl/linewidth.phself.0.000K';
linewidth_file = 'data/BaCoAl/linewidth.phself.300.000K';
...
%
% ==== 2. 读取 lambda 文件 ====
% lambda_file = 'data/BaCoAl/lambda_0k.dat';
lambda_file = 'data/BaCoAl/lambda_300k.dat';
...
% ==== 4. 保存结果 ====
outfile = 'data/BaCoAl/a2F_qv.dat';
fid = fopen(outfile, 'w');
...
data = readmatrix('data/BaCoAl/meshes/kpoints2.dat'); 
kpath = data(:,1);
kpath(142:end)=kpath(142:end)-0.36;       
kpath(162:end)=kpath(162:end)-0.78;

labels={'\Gamma','M','K','\Gamma','A','L','H','A|L','M|H','K'}; % labels for k
kk=[0.000000,0.393194,0.696554,1.208973,2.005542,2.398737,2.702096,3.214515,4.0065,4.7985];
linesize=2;
xrange=[kpath(1),kpath(end)];
figure();
Energy=omega_matrix*0.2418;
...
%%
% data = readmatrix('data/BaCoAl/a2f_w_0k.dat');   % 第一列: omega, 第二列: a2F(omega)
data = readmatrix('data/BaCoAl/a2f_w_300k_mesh.dat');   % 第一列: omega, 第二列: a2F(omega)
omega = data(:,1); a2f = data(:,2);
...
% 计算并绘制 λ 的累积值
figure;
plot(omega*0.2418, lambda_cum, 'b-', 'LineWidth', 1.5);
hold on;
plot(omega*0.2418,f_smooth,'b--','LineWidth',1.5)
plot(omega*0.2418,f_smooth_ori,'r-','LineWidth',1.5)
xlabel('\omega (THz)');
ylabel('\lambda_{cum}');
title('Cumulative \lambda vs \omega');
grid on;
...
% data = readmatrix('data/BaCoAl/meshes/kpoints2.dat'); 
% kpath = data(:,1);
% kpath(142:end)=kpath(142:end)-1;
% kpath(162:end)=kpath(162:end)-0.8;
% 
% labels={'\Gamma','M','K','\Gamma','A','L','H','A|L','M|H','K'}; % labels for k
% kk=[0.000000,1.047198,1.855162,3.2199,3.534074,4.581272,5.389236,6.753990,7.099756,7.4595];
...
labels={'\Gamma','M','K','\Gamma','A','L','H','A|L','M|H','K'}; % labels for k
kk=[0.000000,0.393194,0.696554,1.208973,2.005542,2.398737,2.702096,3.214515,4.0065,4.794];
...
lambda_flat = lambda_matrix(:);
a2f_flat=a2f_matrix(:);
circle_size = 400 * abs(lambda_flat+0.000001);  % 调节可视大小

h0=scatter(ax1, q_flat, omega_flat*0.2418, circle_size, ...
    'filled', 'MarkerFaceAlpha', 0.5, ...
    'MarkerFaceColor', [0.2 0.4 0.9]);
...
h2=plot(ax2, f_smooth*10,omega*0.2418,'r-','LineWidth',1.5);
lgd2=legend(ax2, h2, { '$\sum_{qv}\lambda_{qv}\delta(\omega-\omega_{qv})$'}, ...
       'Interpreter','latex', 'Location','best');
lgd2.FontSize = 10;
hold on;
h3=plot(ax2, lambda_cum,omega*0.2418,'b-','LineWidth',1.5);
% h3=plot(omega*0.2418,a2f,'b-','LineWidth',1.5)
...
xlabel(ax2,'\lambda');
xlim(ax2,[0, 0.62])
xticks(ax2,[0,0.2,0.4,0.6])
% title(ax2,'Cumulative \lambda vs \omega');
ax2.FontSize = 10;
set(ax1,'Units','normalized','Position',[0.08 0.02 0.62 0.9]); % 左：更宽
set(ax2,'Units','normalized','Position',[0.72 0.02 0.12 0.9]); % 右：更窄
legend boxoff 