...
hold on;
plot(kpath,Energy(37,:)-efermi,'Color','blue','LineWidth',2);
plot(kpath,Energy(38,:)-efermi,"Color",'red','LineWidth',2);
...
% surf(Kx, Ky, Enk(:,:,3));
hold on;
% surf(Kx, Ky, Enk(:,:,6)-Enk(:,:,5));
elist=linspace(-1.34,-1.32,20)
contour3(Kx, Ky, Enk(:,:,38),elist, 'LineColor', 'k', 'LineWidth', 0.5);
colormap(slanCM('RdBu'))
colormap(flipud(colormap));
shading interp
% axis equal
% 自定义颜色条范围
caxis([-1.34,-1.32]); % 设置颜色范围（单位: eV）

% h = colorbar;
% h.Ticks = [0, 1, 2, 3, 4, 5]; % 自定义颜色条刻度
% h.Label.String = 'Band (eV)'; % 设置颜色条标签
...
material dull
% 设置图形标签与标题
xlabel('Kx', 'FontSize', 12);
% ylabel('r (Å)', 'FontSize', 12);
ylabel('Ky', 'FontSize', 12);
title('Band structure of RhG-5s', 'FontSize', 14);
grid off;
% 调整显示
...
function fplo=read_fplo(name)
        g = MTB.geometry(name);
        g = MTB.read_poscar(g,"data/InSe/fplo/POSCAR");
        pos=textread("data/InSe/fplo/wpos");
        g.wpos=pos;
        ham=textread("data/InSe/fplo/mydata-p1");
        tol = 1e-9;  % 容差，避免浮点误差
