...
load("rhg_15s.mat");
p=params_fit;
V_pi=p(1); V_sigam=p(2); t2=p(3); visp_val=p(4);
l0=p(5); q_pi=p(6); a_pi=p(7); q_sigma=p(8); a_sigma=p(9); r_c=p(10); l_c=p(11);
%creat the 15s tb modles
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%      Calculate the Dos on E field and Chemical Potential          %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
clc;
clear;
N=15;
a0=2.46; c = 62; d0=3.35;
load("rhg_15s.mat");
p=params_fit;
V_pi=p(1); V_sigam=p(2); t2=p(3); visp_val=p(4);
l0=p(5); q_pi=p(6); a_pi=p(7); q_sigma=p(8); a_sigma=p(9); r_c=p(10); l_c=p(11);

E_list=linspace(0,1e-1,41);% 4100
Enum=21; %1001
...
parfor E_idx=1:length(E_list)
    ...
    Dos=Dos*(Emax-Emin)/Enum;
    % density=TDos/norm(cross(g.a(1,:),g.a(2,:)))*10^16*(0.08)^2*6;
    % data(E_idx,:,1)=Dos;
    % data(E_idx,:,2)=density;
    % 统一行向量并写入
    Dos_all(E_idx,:) = Dos(:).';
    TDos_all(E_idx,:) = TDos(:).';
    toc;
...
E_list=linspace(0,3e-3,1201)*46.9;
...
params0 = [V_pi , V_sigam, t2, visp_val,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];              % 初始猜测
% params0 = params_fit;
% params0 = [ -2.9540 0.4600    0.0021  1e-3,];
lb = [V_pi-0.6 , V_sigam-0.2, t2-0.01, -0.01,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 下界
ub = [V_pi+0.6,  V_sigam+0.2, t2+0.01, 0.01,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 上界
% lb = params0-abs(params0)*0.1
% ub = params0+abs(params0)*0.1
...
residual_fun = @(p) reshape( weights .* abs((get_sk_bands(p) - E_DFT)).^2, [], 1 );
...
optsPSO = optimoptions('particleswarm', ...
    ...
    'UseParallel',        true, ...
    'Display',            'iter', ...
    'MaxIterations',      100, ...     % 最大迭代轮数
    'MaxStallIterations', 20, ...     % 允许连续"几乎没进展"的轮数
    'FunctionTolerance',  1e-8, ...    % 收敛阈值（越小越严格）
    'ObjectiveLimit',     1e-6, ...    % 达到该目标值即停止（可选）
    'MaxTime',            3600);       % 最长运行1小时（可选）

obj = @(p) mean( (get_sk_bands(p) - E_DFT).^2 , 'all');
params0 = [V_pi , V_sigam, t2, visp_val,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];              % 初始猜测
params0=params_fit;
% params0 = [ -2.9540 0.4600    0.0021  1e-3,];
% % lb = [V_pi-0.4 , V_sigam-0.2, t2-0.01, -0.0005,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 下界
% % ub = [V_pi+0.4,  V_sigam+0.2, t2+0.01, 0.0005,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 上界
lb = params0-abs(params0)*0.1;
ub = params0+abs(params0)*0.1;
dim  = numel(lb);
% residual_fun = @(p) reshape( weights .* abs((get_sk_bands(p) - E_DFT)), [], 1 );
% [params_fit, ~, ~, exitflag, output] = lsqnonlin(residual_fun, params0, lb, ub, opts); %#ok<ASGLU>
[params_pso, fval_pso] = particleswarm(obj, dim, lb, ub, optsPSO);


%%

params0 = [V_pi , V_sigam, t2, 0.0001,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];              % 初始猜测
params_0=params_pso;
% params0 = [ -2.9540 0.4600    0.0021  1e-3,];
% lb = [V_pi-0.4 , V_sigam-0.2, t2-0.01, -0.0005,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 下界
% ub = [V_pi+0.4,  V_sigam+0.2, t2+0.01, 0.0005,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];                    % 上界
lb = params0-abs(params0)*0.05;
ub = params0+abs(params0)*0.05;
...
%% ====================== 2. 运行拟合 ======================
[params_fit, ~, ~, exitflag, output] = lsqnonlin(residual_fun, params_0, lb, ub, opts); %#ok<ASGLU>
disp('===== 拟合完成 =====');
disp(['params_fit = [t, Delta] = [', num2str(params_fit(1),'%.6f'), ', ', num2str(params_fit(2),'%.6f'), ']']);
disp(['exitflag = ', num2str(exitflag)]);
disp(output.message);

% （可选）PSO 结果作为初值，用 lsqnonlin 做局部精修
...
% % 
% % [theta_refined, ~] = lsqnonlin(@(x) bandfit_residual(x, target, klist, g), ...
% %                                theta_pso, lb, ub, optsLSQ);
%%
bands_sk_pso=get_sk_bands(params_pso);
bands_sk=get_sk_bands(params_fit);
% plot(bands_data(:,1),bands_data(:,2),'r.',LineWidth=2)
figure()
hold on;
for i=1:size(bands_vasp,2)
    plot(kpath_vasp(:,i),bands_vasp(:,i),'b-',LineWidth=2)
    plot(kpath_vasp(:,i),bands_sk(:,i),'r--',LineWidth=2)
    plot(kpath_vasp(:,i),bands_sk_pso(:,i)-0.002,'ko',LineWidth=0.5)
end
ylim([-0.1,0.1])
...
g = MTB.read_poscar(g,"data/Graphene/15s/POSCAR");
[g.ham,g.hopr] = MTB.wannier.read_hr('data/Graphene/15s/wannier90_hr_p1.dat','data/Graphene/15s/wannier90_hr_p2.dat');
%%
% % filename='data/Graphene/15s/wb_tb.dat'
% % MTB.write_wb_hr(g,filename)
%%
...
t2=p(3);
visp_val=p(4);
l0=p(5); q_pi=p(6); a_pi=p(7); q_sigma=p(8);
a_sigma=p(9); r_c=p(10); l_c=p(11);
parameters_tsk=[V_pi,V_sigam,l0,q_pi,a_pi,q_sigma,a_sigma,r_c,l_c];
shells=[2,5,6,21];
...
bands_sk=Energy';
efermi=min(bands_sk(:,16));
bands_sk=bands_sk(:,15:16)-efermi;
...
%   - 使用 'unique(...,"rows","stable")' 保持首出现顺序稳定。
    ...
    Visp = -diag(abs(g.wpos(:, 3)-mean(g.wpos(:,3))) * z_slope);
