function [Energy,kpath,kk]=get_slab_bands_sparse_BdG(hamiltonian,hopping_r,nslab,nbands,nrpts,hkpoints,nk,a,b,mu,delta)
...
%        fprintf('Job on k_i = %2d time: %8.4f s\n',i,T(i))
        vals=sort(real(eigs(hk,10,'smallestabs')));
        Energy(:,i)=vals;
    end
   % close(hbar);
end
...
function Hij=ham_qlayer2qlayer(hamiltonian,hopping_r,nbands,nrpts,kpoint,a)
    ...
    Hij=sparse(1,1,0,nbands*nrpts,nbands*nrpts)
    for j=1:nrpts
        if  (ijmin<= hopping_r(j,3) <= ijmax)
            kdotr = dot(kpoint,hopping_r(j,1:2)*a);
            ratio = exp(1j*kdotr);
            hk_r=hamiltonian(:,:,j)*ratio;
            hk_r=sparse(hk_r)
            Hij(1:nbands,1:nbands)=hk_r
